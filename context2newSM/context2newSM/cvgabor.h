...
*
*
*   2010-08 32F画像でも使えるように変更 by Kohei Mashimo
*
***************************************************************************/
#ifndef CVGABOR_H
#define CVGABOR_H
...
void CvGabor::conv_img(IplImage *src, IplImage *dst, int Type)
{
	...
	if(src->depth == IPL_DEPTH_8U){
		//cout << "Gabor::Input Image Type is IPL_DEPTH_8U" <<endl;
		for (int i = 0; i < src->width; i++){
			for (int j = 0; j < src->height; j++){
				//ve = CV_IMAGE_ELEM(src, uchar, j, i);
				ve = cvImageElem<IPL_DEPTH_8U>(src, i, j, 0);
				CV_MAT_ELEM(*mat, float, i, j) = (float)ve;
			}
		}
	}
	if(src->depth == IPL_DEPTH_32F){
		//cout << "Gabor::Input Image Type is IPL_DEPTH_32F" <<endl;
		for (int i = 0; i < src->width; i++){
			for (int j = 0; j < src->height; j++){
				//ve = CV_IMAGE_ELEM(src, float, j, i);
				//cout << "src->height::" << src->height <<endl;
				//cout << "i,j::" << i << "," <<j<<endl;
				ve = cvImageElem<IPL_DEPTH_32F>(src, i, j, 0);
				CV_MAT_ELEM(*mat, float, i, j) = (float)ve;
			}
		}
	}
